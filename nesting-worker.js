...
function findPositionWithNFPWorker(placedParts, newPart, sheetWidth, sheetHeight, minGap, cancelCallback) {
    ...
    for (let i = 0; i < rotationAngles.length; i++) {
        ...
        for (let y = minGap; y <= sheetHeight - rotatedBbox.height - minGap; y += step) {
            if (cancelCallback && cancelCallback()) {
                return null;
            }
            
            for (let x = minGap; x <= sheetWidth - rotatedBbox.width - minGap; x += step) {
                const positionedHull = translatePolygonWorker(normalizedHull, x, y);

                if (!isPolygonInsideSheetWorker(positionedHull, sheetWidth, sheetHeight, minGap)) {
                    continue;
                }

                let canPlace = true;
                for (const placed of placedParts) {
                    if (polygonsIntersectWorker(positionedHull, placed.positionedHull, minGap)) {
                        canPlace = false;
                        break;
                    }
                }
