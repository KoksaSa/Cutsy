...
function updatePartsList() {
    ...
    try {
        ...
        if (!list) {
            console.warn('[updatePartsList] Элемент #partsList не найден');
            return;
        }
    ...
    `}).join('');
        ...
        list.querySelectorAll('.nesting-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
                ...
                console.log(`[updatePartsList] Раскладка детали "${part.name}": ${checkbox.checked ? 'ВКЛ' : 'ВЫКЛ'}`);
                updatePartsList();
                ...
                if (autoNestingCheckbox && autoNestingCheckbox.checked) {
                    console.log('🚀 Авторасскладка (чекбокс детали): запуск раскладки...');
                    setTimeout(async () => {
                        try {
        ...
        list.querySelectorAll('input[type="number"]').forEach(input => {
            ...
            input.addEventListener('change', (e) => {
                ...
                if (part) {
                    ...
                    part.quantity = qty;
                    if (typeof saveToCache === 'function') saveToCache();
                    console.log(`[updatePartsList] Количество детали "${part.name}" изменено на ${part.quantity}`);

                    // ─── АВТОРАСКЛАДКА — если включена ───────────────
                    const autoNestingCheckbox = document.getElementById('autoNestingCheckbox');
                    if (autoNestingCheckbox && autoNestingCheckbox.checked) {
                        console.log('🚀 Авторасскладка (quantity): запуск раскладки...');
                        setTimeout(async () => {
                            try {
                                const nestBtn = document.getElementById('nestMultiParts');
                                if (nestBtn && typeof nestBtn.onclick === 'function') {
        ...
        list.querySelectorAll('.part-thickness-input').forEach(input => {
            input.addEventListener('change', (e) => {
                ...
                if (typeof saveToCache === 'function') saveToCache();
                console.log(`[updatePartsList] Толщина детали "${part.name}" изменена на ${thickness} мм`);
                updatePartsList();
            });
        ...
        list.querySelectorAll('.part-spacing-input').forEach(input => {
            input.addEventListener('change', (e) => {
                ...
                if (typeof saveToCache === 'function') saveToCache();
                console.log(`[updatePartsList] Отступ детали "${part.name}" изменён на ${spacing} мм`);
                updatePartsList();

                // ─── АВТОРАСКЛАДКА — если включена ───────────────
                const autoNestingCheckbox = document.getElementById('autoNestingCheckbox');
                if (autoNestingCheckbox && autoNestingCheckbox.checked) {
                    console.log('🚀 Авторасскладка (spacing): запуск раскладки...');
                    setTimeout(async () => {
                        try {
                            const nestBtn = document.getElementById('nestMultiParts');
                            if (nestBtn && typeof nestBtn.onclick === 'function') {
...
(function initThicknessFilter() {
    ...
    if (select) {
        select.addEventListener('change', (e) => {
            ...
            updatePartsList();
            console.log(`[Thickness Filter] Выбрана толщина ${thickness}мм`);
        });
    }
...
window.viewPart = function(partId) {
    ...
    if (_viewingPart) {
        console.warn('[viewPart] Повторный вызов заблокирован');
        return;
    }
    _viewingPart = true;
    ...
    try {
        ...
        if (part.visible) {
            ...
            console.log(`[viewPart] Скрываем деталь #${partId} "${part.name}"`);
            ...
            console.log(`[viewPart] Удалено объектов: ${beforeCount - objects.length}`);
        }
        // Если включаем видимость детали
            ...
            if (visibleParts.length > 0) {
                ...
                console.warn(`[viewPart] Уже видима деталь #${visibleParts[0].id}`);
            ...
            console.log(`[viewPart] Показываем деталь #${partId} "${part.name}"`);
            ...
            console.log(`[viewPart] Добавлено объектов: ${part.objects.length}`);
        }
...
window.deletePart = function(partId) {
    ...
    if (part) {
        ...
        if (removedNested > 0) {
            console.log(`[deletePart] Удалено ${removedNested} размещённых копий из nestedParts`);
        }
        ...
        if (window.allSheets && window.allSheets.length > 0) {
            ...
            if (totalRemovedFromSheets > 0) {
                console.log(`[deletePart] Удалено ${totalRemovedFromSheets} размещённых копий из allSheets`);
            }
        }
    ...
    console.log(`[deletePart] Деталь #${partId} удалена`);
};
...
window.changePartSpacing = function(partId, value) {
    ...
    if (isNaN(spacing) || spacing < -100 || spacing > 100) {
        console.warn(`[changePartSpacing] Значение ${spacing} вне диапазона [-100;100]`);
        updatePartsList();
        return;
    }
    ...
    part.spacing = spacing;
    if (typeof saveToCache === 'function') saveToCache();
    console.log(`[changePartSpacing] Деталь "${part.name}" -> отступ: ${spacing} мм`);
    updatePartsList();
};
...
window.toggleNoRotate = function(partId) {
    ...
    console.log(`[toggleNoRotate] Деталь "${part.name}" -> Не вращать: ${part.noRotate ? 'ВКЛ' : 'ВЫКЛ'}`);
...
window.toggleAllowedAngle = function(partId, angle) {
    ...
    console.log(`[toggleAllowedAngle] Деталь "${part.name}" -> углы: [${part.allowedAngles.length > 0 ? part.allowedAngles.join(', ') : 'авто'}]`);
    ...
    const autoNestingCheckbox = document.getElementById('autoNestingCheckbox');
    if (autoNestingCheckbox && autoNestingCheckbox.checked) {
        console.log('🚀 Авторасскладка (углы): запуск раскладки...');
        setTimeout(async () => {
            try {
                const nestBtn = document.getElementById('nestMultiParts');
                if (nestBtn && typeof nestBtn.onclick === 'function') {
...
window.resetAllowedAngles = function(partId) {
    ...
    part.allowedAngles = [];
    console.log(`[resetAllowedAngles] Деталь "${part.name}" -> углы сброшены`);
    ...
    const autoNestingCheckbox = document.getElementById('autoNestingCheckbox');
    if (autoNestingCheckbox && autoNestingCheckbox.checked) {
        console.log('🚀 Авторасскладка (сброс углов): запуск раскладки...');
        setTimeout(async () => {
            try {
                const nestBtn = document.getElementById('nestMultiParts');
                if (nestBtn && typeof nestBtn.onclick === 'function') {
