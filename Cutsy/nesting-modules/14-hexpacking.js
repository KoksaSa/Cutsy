// ════════════════════════════════════════════════════════════════
// SilikinK Nesting Engine — Hexagonal Packing for Circles (Module 14)
// v3.83: Circle-in-Ring multi-circle triangle arrangement
// ════════════════════════════════════════════════════════════════
(function(N) {
    'use strict';
...
N.tryHexagonalPacking = function tryHexagonalPacking(normalizedHull, refPoint, bbox, newPart, placedParts, sheetWidth, sheetHeight, minGap, edgeGap, cancelCallback, spatialGrid) {
    ...
    // 2) Позиции вокруг уже размещённых круглых деталей (cross-type)
    const hexCandidates = [];
    ...
    // ═══════════════════════════════════════════════════
    // v3.81: NEAR-EXISTING-CIRCLES кандидаты
    // Для каждого размещённого круглого элемента генерируем
    // позиции вокруг него на правильном расстоянии.
    // Это позволяет маленьким кругам размещаться вплотную
    // к большим, а не только на стандартной гекс-сетке.
    // ═══════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════
    // v3.81: INTERSTITIAL кандидаты
    // Для каждой пары близких круглых деталей вычисляем
    // позицию между ними, где может поместиться новый круг.
    // Это ключевой механизм для плотной упаковки кругов
    // разного размера: маленькие круги встают в "щели"
    // между большими.
    // ═══════════════════════════════════════════════════
    for (let i = 0; i < placedCircles.length && hexCandidates.length < MAX_HEX_CANDIDATES; i++) {
        ...
        for (let j = i + 1; j < placedCircles.length && hexCandidates.length < MAX_HEX_CANDIDATES; j++) {
            ...
            // треугольник сo сторонами dist1, dist2, d существует
            if (d >= dist1 + dist2 || d <= Math.abs(dist1 - dist2) + 0.01) continue;

            // Вычисляем центр(ы) нового круга — пересечение двух
            // окружностей: радиус dist1 из (cx1,cy1), радиус dist2 из (cx2,cy2)
    ...
    // ═══════════════════════════════════════════════════
    // Стандартная гекс-сетка (fallback)
    // ═══════════════════════════════════════════════════
    let rowNum = 0;
...
// ════════════════════════════════════════════════════════════════
// v3.81: Circle Quick-Place — Cross-type + Interstitial
// Для круглых деталей: размещаем следующую деталь
// рядом с уже размещёнными на гексагональном смещении.
//
// Стратегия (v3.81 — расширена):
// 1) Если есть детали того же типа — гекс-позиции вокруг
//    последней (как раньше)
// 2) Если таких же нет — используем ВСЕ круглые детали
//    как точки привязки (cross-type anchors)
// 3) Для каждой пары близких круглых деталей генерируем
//    интерстициальные позиции (между ними)
// 4) Выбираем позицию с лучшим BLF-score
//
// Это позволяет маленьким кругам вставать вплотную к
// большим и заполнять промежутки между ними.
// ════════════════════════════════════════════════════════════════
N.tryCircleQuickPlace = function tryCircleQuickPlace(normalizedHull, refPoint, bbox, newPart, placedParts, sheetWidth, sheetHeight, minGap, edgeGap, cancelCallback, spatialGrid) {
    ...
    // Находим все размещённые детали того же типа
    ...
    // ═══════════════════════════════════════════════════
    // A) SAME-TYPE: Гекс-позиции вокруг последнего такого же
    // (как в v3.80, но без раннего возврата при 0 штук)
    // ═══════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════
    // B) CROSS-TYPE: Позиции вокруг круглых деталей ДРУГОГО типа
    // Когда нет same-type или они не дали результатов,
    // маленькие круги могут встать вплотную к большим.
    // Для каждого размещённого круга генерируем позиции
    // на расстоянии newRadius + existingRadius + minGap.
    // ═══════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════
    // C) INTERSTITIAL: Позиции между парами круглых деталей
    // Для каждой пары близких круглых деталей вычисляем
    // позицию в "щели" между ними. Маленький круг может
    // встать между двумя большими или между большим и малым.
    //
    // Геометрия: пересечение двух окружностей с центрами
    // (cx1,cy1,r1+newR+gap) и (cx2,cy2,r2+newR+gap).
    // ═══════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════
// v3.85: CIRCLE-IN-RING Quick-Place — Multi-circle arrangement
// Размещение маленьких круглых деталей ВНУТРИ отверстий
// уже размещённых кольцевых деталей (концентрические круги).
//
// ИСТОРИЯ ПРОБЛЕМ:
// v3.82: 1 круг в центре кольца → центр блокирует другие позиции
// v3.83: Треугольник из 3 кругов (без центра) → но gridsOverlap
//         давал ложное перекрытие у границы отверстия из-за
//         дискретизации grid (gridSize=10мм, зазор=10мм = 1 ячейка)
// v3.84: minGap=0 для gridsOverlap → всё равно ложное перекрытие
//         (граница круга в grid размывается на 1 ячейку)
// v3.85: МАТЕМАТИЧЕСКАЯ ПРОВЕРКА вместо gridsOverlap!
//         - Реальный innerR из DXF-объектов (не bbox-оценка)
//         - Проверка: dist(center, holeCenter) + newR + minGap ≤ innerR
//         - Точная, не зависит от разрешения grid
//         - fitD уменьшен на 3мм для запаса (grid-артефакты
//           на границе кольца могут не совпадать с геометрией)
//
// Конфигурации:
// - 3 круга (треугольник, 120°) — если maxD ≥ minD3
// - 2 круга (бок о бок, 180°) — если maxD ≥ minD2
// - 1 круг (центр) — если даже 2 не помещаются
// ════════════════════════════════════════════════════════════════
N.tryCircleInRingPlace = function tryCircleInRingPlace(normalizedHull, refPoint, bbox, newPart, placedParts, sheetWidth, sheetHeight, minGap, edgeGap, cancelCallback, spatialGrid) {
    ...
    for (const placed of placedParts) {
        ...
        // Bbox-оценка (holeW/2) может завышать innerR, т.к. bbox прямоугольный,
        // а отверстие круглое — углы bbox выходят за пределы круга.
        // Реальный innerR = радиус внутренней окружности из DXF-данных.
        ...
        if (partCircles.length >= 2) {
            ...
            // Меньший радиус = внутренний (отверстие), больший = внешний (кольцо)
        ...
        // вместо rotated bbox (placed.width/height). Для круглых деталей они совпадают,
        // но для circle+text или несимметричных колец placed.width/height — это rotated
        // bbox, который при 90°/270° даёт swap W↔H → центр вращения смещается.
        ...
        // ═══════════════════════════════════════════════════
        // v3.85: Определяем, сколько кругов помещается в отверстие
        // и генерируем позиции в ОПТИМАЛЬНОЙ конфигурации.
        //
        // Ключевое изменение: используем actualInnerR из DXF-данных,
        // а не bbox-оценку. Также добавлен запас 3мм к fitD для
        // компенсации неточностей на границе кольца.
        // ═══════════════════════════════════════════════════
        ...
        if (fitCount >= 3) {
            ...
            // Углы: 210° (нижний-левый), 330° (нижний-правый), 90° (верхний)
            // Это даёт лучшие BLF-позиции: два нижних раньше верхнего
    ...
    for (const cand of candidates) {
        ...
        for (const other of toCheck) {
            ...
            if (other === ringPart) {
                ...
                // границе отверстия: при gridSize=10мм и зазоре 10мм
                // ячейки на границе помечены как "материал" → ложное
                // перекрытие → ВСЕ позиции треугольника отклоняются,
                // кроме 90° (которая случайно проходит из-за rounding).
                //
                // Математическая проверка: малый круг полностью внутри
                // отверстия ↔ dist(центр_круга, центр_отверстия) +
                // newRadius + minGap ≤ innerR
                // Это ТОЧНАЯ проверка, не зависит от grid.
                // ═══════════════════════════════════════════════════
                const nCx = x + newRadius;
                const nCy = y + newRadius;
            ...
            if (other.part && N.isCircularPart(other.part)) {
                ...
                // проверяем: малый круг внутри отверстия другого кольца?
                ...
                if (otherConcentric) {
                    ...
                    // Вычисляем центр отверстия другого кольца на листе
                    ...
                    // вместо rotated bbox, как в fix #59 выше.
                    ...
                    // но используем innerR вместо outerR для более точной проверки
                    // (кольцо — это не сплошной круг, а ободок)
                    if (dist < newRadius + oActualInnerR + minGap - 0.01) {
                        // Внутри внутреннего радиуса — это пустота кольца, ОК
                        // Но мы уже проверили insideOHole выше и это не так
                        // Значит малый круг частично внутри ободка → блокируем
                        // Используем ВНЕШНИЙ радиус для проверки
