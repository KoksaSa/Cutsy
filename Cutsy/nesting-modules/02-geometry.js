// ════════════════════════════════════════════════════════════════
// SilikinK Nesting Engine — Geometry Primitives (Module 02)
// ════════════════════════════════════════════════════════════════
(function(N) {
    'use strict';
...
// Для positionedHull и других стабильных полигонов даёт +10-20% скорости.
...
// Prevents stack overflow from enormous segment counts for arcs
// with huge radii (R > 10000mm) that are nearly straight.
// Old formula: Math.ceil(r / gridSize * 2) → 453K segments for R=680658!
...
// когда r <= tolerance (Math.acos(arg outside [-1,1]) = NaN).
// Все вызовы Math.acos(1 - tolerance/r) заменены на эту функцию.
...
// ─────────────────────────────────────────────────────────────
// v3.59: Chain-linking — единый алгоритм сборки сегментов в цепочки.
// Раньше этот же код копипастился 4 раза:
//   - N.extractConcaveOutline (основной контур)
//   - N.extractConcaveOutline (second-shape detection)
//   - getPartPolygons
//   - countContoursFromSegments
//   - countClosedContours
// При баге в merge tolerance приходилось править в 4–5 местах.
//
// Опции:
//   tolerance          — порог соединения концов (default N.CHAIN_TOLERANCE)
//   includeArcPoints   — true: использовать _arcPts для дуг (полилинии контура)
//                        false: только x1/y1/x2/y2 (для подсчёта проколов)
//   maxChains          — сколько максимум цепочек строить (default Infinity).
//                        1 = только первая (extractConcaveOutline: основной
//                        контур, остальные сегменты остаются «неиспользованными»
//                        для second-shape check).
//   startFrom          — индекс предпочтительного стартового сегмента
//                        (extractConcaveOutline: сегмент, максимально удалённый
//                        от центра bbox, чтобы не начать с дырки).
//
...
//                  Остальные индексы = «неиспользованные».
// ─────────────────────────────────────────────────────────────
N.chainLinkSegments = function chainLinkSegments(segments, options = {}) {
    ...
    while (used.size < segments.length && chains.length < maxChains) {
        ...
        // (Конечная точка уже внутри arcPts[length-1] — не дублируем.)
        if (hasArcPts) {
...
// ─────────────────────────────────────────────────────────────
// SAT (Separating Axis Theorem) — быстрая проверка для выпуклых
// полигонов. O(V1+V2) вместо O(V1×V2) для edge-intersection.
// Возвращает: true = точно НЕ пересекаются (разделяющая ось найдена),
...
// v3.39: Кешированный bbox + SAT для выпуклых + оптимизация gap-чека
N.polygonsIntersect = function polygonsIntersect(poly1, poly2, minGap = 3) {
    ...
    // FIX v3.49: Используем +gap вместо -gap.
    if (poly1.length <= 8 && poly2.length <= 8) {
        if (N.satDisjoint(poly1, poly2, gap)) return false;
    }

    // ═══════════════════════════════════════════════════════════
    // v3.58: При отрицательном minGap — полигоны МОГУТ пересекаться,
    // но глубина проникновения не должна превышать -minGap.
    // Алгоритм: если есть пересечение, вычисляем глубину
    // проникновения через SAT с отрицательным gap.
    // ═══════════════════════════════════════════════════════════
    if (minGap < 0) {
        ...
        // SAT с gap = minGap (отрицательное значение) — если проекции
        // разделились больше чем на |minGap|, то глубина проникновения
        // больше допустимой → столкновение.
        // N.satDisjoint(poly1, poly2, minGap) при minGap < 0:
        //   overlap < -minGap → разделяющая ось → нет значительного проникновения → false
        //   overlap >= -minGap → значительное проникновение → true
        // Но SAT может пропустить невыпуклые формы → для >8 вершин
        // используем vertex-to-edge расстояние.
        const maxOverlap = -minGap; // допустимая глубина проникновения в мм

        if (poly1.length <= 8 && poly2.length <= 8) {
            // SAT для выпуклых — если SAT не нашёл разделяющую ось
            // с gap=minGap, значит проникновение > maxOverlap
            if (!N.satDisjoint(poly1, poly2, minGap)) return true;
            return false;
        }

        // Для невыпуклых — вычисляем минимальное vertex-to-edge расстояние
        // для пересекающихся вершин. Если хотя бы одна вершина проникает
        // глубже maxOverlap → столкновение.
        ...
        for (let i = 0; i < outer.length; i++) {
            ...
            for (let j = 0; j < inner.length; j++) {
                ...
                // Если dist > maxOverlap — проникновение слишком глубокое
    ...
    if (minGap > 0) {
        ...
        // к рёбрам poly2 И вершины poly2 к рёбрам poly1. Раньше
        // проверялись только вершины «меньшего» полигона к рёбрам
        // «большего» — при определённых ориентациях gap violation
        // на «дальней» стороне пропускался.
        for (let i = 0; i < poly1.length; i++) {
            for (let j = 0; j < poly2.length; j++) {
