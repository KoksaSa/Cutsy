// ════════════════════════════════════════════════════════════════
// SilikinK Nesting Engine — Main Position Finder (Module 15)
// ════════════════════════════════════════════════════════════════
(function(N) {
    'use strict';
...
N.findPositionWithNFP = async function findPositionWithNFP(placedParts, newPart, sheetWidth, sheetHeight, cancelCallback = null, spatialGrid = null) {
    ...
    if (newPart.oneCutEnabled === true && placedParts.length > 0) {
        ...
        // Пользователь может установить отрицательный spacing (-100..+100)
        // для перекрытия bounding box деталей (материал защищён
        // polygonsIntersect/gridsOverlap с Math.max(0, minGap)).
        // Ограничиваем только диапазон:
        minGap = Math.max(-100, Math.min(100, minGap));
    }

    // ═══════════════════════════════════════════════════
    // ДИАГНОСТИКА SPACING: логируем источник зазора
    // ═══════════════════════════════════════════════════
    {
    ...
    // ═══════════════════════════════════════════════════
    // HOLE QUICK-PLACE: пробуем разместить деталь внутри
    // пустот (дыр) уже размещённых больших деталей.
    // Приоритетнее обычного Quick-Place — заполняем пустоты
    // внутри деталей, а не свободное пространство листа.
    // Использует occupancy grid для точной проверки
    // (polygonsIntersect даёт ложные срабатывания для кругов).
    // v3.2: фильтр по размеру дыры, сортировка, ранний выход
    // ═══════════════════════════════════════════════════
    if (placedParts.length > 0 && !isCircular) {
        // Пробуем углы: batch-углы + 0° + перпендикулярный
        // FIX: учитываем allowedAngles/getRotationAngles для hole-размещения,
        ...
        // Это исключает сканирование мелких деталей (Д, уг) чьи дыры
        // слишком малы для размещения чего-либо.
        ...
        // (для любого угла — берём минимум из двух ориентаций)
        ...
        for (const placed of placedParts) {
            ...
            // тоже могут иметь отверстия (кольца, рамки). Раньше
            // повёрнутые детали пропускались → отверстия не заполнялись.
            // Теперь координаты дыры пересчитываются с учётом поворота.
            const partRef = placed.part;
            if (!partRef) continue;

            // v3.73: Определяем, есть ли у детали отверстия для размещения.
            // Раньше требовалось hasSolidFill || hasConcentricCircles —
            // детали с простыми монтажными отверстиями (боковина: 28 кругов r=2..3)
            // пропускались, хотя detectPartHoles теперь находит эти отверстия.
            // Теперь: проверяем getPartHoles() — если есть обнаруженные отверстия,
            ...
            if (!hasSolid) {
                ...
                // [Hole Quick-Place] circles=${pCircles.length}, hasSolidFill=${hasSolid}
            ...
            // деталь хотя бы в одном углу поворота
            // v3.4: Фильтр "подозрительно больших" дыр.
            // v3.42: Порог снижен с 0.5 до 0.35 — даже 35% площади
            // «дыра» подозрительна и скорее всего артефакт grid.
            // Для концентрических кругов (кольцо) — дыра занимает
            // >50%, но это РЕАЛЬНОЕ отверстие → пропускаем фильтр.
            // Также: если isSuspicious=true — пропускаем (определено
            // в detectPartHoles через filled grid).
            const partBbox = partRef.bounds;
            // v3.52: Используем реальный gridSize из getPartHoles вместо
            // захардкоженного 10мм. Раньше gridSize=10 давал заниженный
            // totalGridCells для мелких деталей (gridSize=3) → ложные
            // "подозрительно большие" дыры → отверстия отсекались.
        ...
        // размещены такие же детали (дозаполнение), потом по размеру.
        // Это заполняет каждое кольцо максимально плотно перед переходом
        // к следующему, вместо распределения по одному в каждое кольцо.
        ...
        if (holeCandidates.length > 0) {
            ...
            for (const hAngleDeg of holeAngleDegs) {
                ...
                // поместиться в самую большую дыру?
                ...
                for (const { placed, holes } of holeCandidates) {
                    ...
                    // placedW/placedH — rotated bbox (swap при 90°/270°).
                    // Для центра вращения нужны оригинальные размеры.
                    ...
                    for (const hole of holes) {
                        ...
                        if (Math.abs(placedAngle) > 0.01) {
                            ...
                            // v4.39 FIX #47: используем ОРИГИНАЛЬНЫЕ размеры (origPlacedW/H),
                        ...
                        // v3.98: Для L-образных деталей используем мелкий шаг,
                        // т.к. bbox включает пустой квадрант — реальные L-детали
                        // могут быть ближе друг к другу, чем bbox+gap.
                        ...
                        // разрешаем bbox выходить за пределы отверстия
                        // (пустой квадрант L-детали может быть за пределами
                        // отверстия — это нормально, материал внутри).
                        // isRectInPartHole слишком строгий для L-деталей.
                        ...
                        for (let hy = holeSheetY + minGap; hy + hPrepared.bbox.height <= holeSheetY + effectiveHoleH - minGap + overflowMargin; hy += stepY) {
                            for (let hx = holeSheetX + minGap; hx + hPrepared.bbox.width <= holeSheetX + effectiveHoleW - minGap + overflowMargin; hx += stepX) {
                                ...
                                // целиком в пустой области (дыре) хост-детали
                                // v3.98: Для L-деталей в концентрических отверстиях
                                // isRectInPartHole слишком строгий (требует полный bbox
                                // внутри отверстия, но пустой квадранл L может выходить
                                // за край). Заменяем на gridsOverlap — он проверяет
                                // реальное наложение материала, а не bbox.
                                ...
                                if (allowBboxOverflow) {
                                    ...
                                    // (материал не на материале кольца)
                                ...
                                for (const other of toCheck) {
                                    if (other === placed) {
                                        ...
                                        // Пропускаем hull-check для концентрических кругов
                                        // и прямоугольных hull (4 вершины).
                                        //
                                        // v3.69: Для деталей с _hasHoles — БОЛЬШЕ НЕ
                                        // пропускаем hull-check полностью! Теперь: если
                                        // hull-ы пересекаются, проверяем gridsOverlap.
                                        ...
                                        if (!isConcentric2 && !isConvexRect2 && !hostHasHoles2 && placed.positionedHull?.length >= 3 && phull.length >= 3) {
                                            ...
                                            // или выпуклый прямоугольник — hull-проверка
                                            // ненадёжна (hull обёртывает отверстия/дыры).
                                            // Если hull-ы пересекаются, проверяем gridsOverlap.
                                            // v3.96: SAFETY NET в gridsOverlap теперь учитывает
                                            // концентрические отверстия — не ложно перекрывает
                                            // grid-результат для колец. Поэтому gridsOverlap
                                            // вернёт корректный результат и для кольца.
    ...
    // ═══════════════════════════════════════════════════
    // QUICK-PLACE: try placing next to last same-type part
    // Avoids full generateCandidates for grid-like placement
    // ═══════════════════════════════════════════════════
    if (placedParts.length > 0 && !isCircular) {
        ...
        if (lastSame) {
            ...
            // Если у детали заданы allowedAngles или _batchAngles,
            // и угол lastSame не входит в их число — Quick-Place
            // пропускается (нельзя размещать при неразрешённом угле).
            // Это исправляет проблему: пользователь выбирает 90°, но
            // Quick-Place размещает при 0° (угол первой детали).
        ...
        if (lastSame) {
            ...
            // ═══════════════════════════════════════════════════
            // Quick-Place: попытка разместить рядом с последней
            // такой же деталью. Варианты:
            // 1) «вслед» — правее (тот же ряд)
            // 2) «новый ряд» — ниже, начиная с левого края (edgeGap),
            //    Y = макс. нижняя граница деталей в X-диапазоне нового ряда
            // 3) «под всеми» — ниже ВСЕХ размещённых деталей (смешанные типы)
            // v3.6: Для криволинейных деталей — добавляем позиции
            // с Y-смещением меньше полной высоты (вложение парабол)
            // ═══════════════════════════════════════════════════

            const isLH = N.isLineHeavyPart(newPart);

            // ═══════════════════════════════════════════════════
            // v3.25: ВЛОЖЕНИЕ КРИВОЛИНЕЙНЫХ ДЕТАЛЕЙ
            // Паттерн <<<<<<<< — все детали в одном направлении.
            // Пробуем ДВЕ стратегии и выбираем лучшую (BLF):
            //
            // 1) «вслед:вложение» — тот же угол, горизонтальное
            //    размещение правее предыдущей детали с Y-offset.
            //    X: от minGap до lW+minGap (сканирование).
            //    Y: от lastSame.y до lastSame.y+lH (ступенчатый
            //    паттерн <<<< с взаимовложением выступов и пустот).
            //
            // 2) «вложение:прямо» — тот же угол, вертикальное
            //    размещение на левом краю (новый ряд).
            //    Y-offset с мелким шагом, X = edgeGap.
            //
            // Выбираем вариант с минимальным BLF-score.
            // ═══════════════════════════════════════════════════
            // ═══════════════════════════════════════════════════
            // v3.22: Safety check — вложение only if filled grid is valid
            // v4.24: Для треугольников тоже вычисляем filled grid (даже если
            // не line-heavy) — нужен для grid-based collision при interlocking
            // ═══════════════════════════════════════════════════
            ...
            // nesting scan (вслед:вложение) НЕ запускаем — он приводит
            // к наложению деталей из-за ненадёжного filled grid.
            // Nesting scan предназначен только для Г-образных деталей,
            // где выступ одной детали входит в пустоту другой.
            // Для прямоугольных деталей с пазами это бессмысленно и опасно.
            // v4.24: Также запускаем для ТРЕУГОЛЬНЫХ деталей — они
            // взаимно вкладываются при чередовании 0°/180° (вершина вверх/вниз).
            if (hasValidGrid && (trulyLShaped || isTriangle)) {
                ...
                // ═══════════════════════════════════════════════════════════
                // v3.34: ЧЕРЕДОВАНИЕ ПАРАМИ для Г-образных деталей [0°, 180°]
                // Паттерн: Г⌐ Г⌐ Г⌐ Г⌐ — пары взаимовложенных деталей
                //
                // Логика:
                // 1) Считаем сколько деталей уже размещено при 0° и 180°
                // 2) Если 0° больше чем 180° → размещаем 180° (завершаем пару)
                //    Ищем последнюю 0° деталь и вкладываемся в неё
                // 3) Если 0° = 180° → размещаем 0° (начинаем новую пару)
                //    Ищем правый край текущего ряда и ставим правее
                // ═══════════════════════════════════════════════════════════
                ...
                if (hasInterlockingPair) {
                    ...
                    if (targetAngleDeg === 180 && count0 > count180) {
                        ...
                        if (last0Part) {
                            ...
                            // ═══════════════════════════════════════════════════════════
                            // v4.24: ПРЯМАЯ ПОЗИЦИЯ INTERLOCKING для треугольников
                            // Для треугольника 0° + 180° идеальная позиция:
                            //   x = refX + refW/2, y = refY (same Y, shifted right by half-width)
                            // Треугольники при этом делят сторону (не перекрываются).
                            // Проверяем эту позицию ПЕРВОЙ — если валидна, используем
                            // сразу без медленного сканирования.
                            // ═══════════════════════════════════════════════════════════
                            if (isTriangle) {
                                // v4.31: Аналитическая ideal-позиция для interlocking 0°+180°.
                                // Используем actual apex (rightApexX) вместо W/2 — это даёт
                                // точную позицию для асимметричных треугольников (со скошенной
                                // вершиной или фасками). Для симметричного rightApexX ≈ W/2.
                                //
                                // Формула: dx = rightApexX + minGap * sqrt(H² + (W-rightApexX)²) / H
                                // даёт перпендикулярный зазор minGap между правым ребром 0°
                                // и левым ребром 180° (они параллельны — оба отражения одного ребра).
                                //
                                // ВНИМАНИЕ: это нижняя граница. Реальная позиция может быть
                                // правее из-за corner-to-edge расстояния (фаски на углах).
                                // Если idealX не проходит — fallback scan ниже найдёт позицию.
                                ...
                                if (idealX + targetPrepared.bbox.width <= sheetWidth - edgeGap &&
                                    ...
                                    if (N.isPolygonInsideSheet(idealHull, sheetWidth, sheetHeight, minGap, edgeGap)) {
                                        ...
                                        for (const p of idealToCheck) {
                                            ...
                                            // polygon-проверку вместо gridsOverlap. Grid-based проверка
                                            // может давать ложное срабатывание из-за дилатации и
                                            // конечного разрешения сетки. Polygon-проверка точно
                                            // определяет, что треугольники делят сторону (не перекрываются).
                                        ...
                                        if (idealCanPlace) {
                                            ...
                                            // (код ниже пропустится из-за проверки nestCandidates.length)
                                        }
                                    }
                                }
                            }
                            ...
                            // уже найдена — пропускаем медленное сканирование
                            if (nestCandidates.length === 0) {
                            // v4.31: Тонкий скан начиная с позиции близко к analytical ideal.
                            // Раньше sXMin=refX и sXStep=23мм — пропускали оптимальную позицию
                            // (для треугольника 579мм: optimal dx≈327, а шаг 23 давал 325→348,
                            //  но 325 отвергалось из-за micro-collision, бралось 348 или 394).
                            // Теперь sXMin ближе к apex (refX + 0.45*W), шаг 3мм — находим
                            // первый valid dx за ~20 итераций вместо ~110.
                            ...
                            for (let tryX = sXMin; tryX <= sXMax; tryX += sXStep) {
                                ...
                                for (let tryY = sYMin; tryY <= sYMax; tryY += sYStep) {
                                    ...
                                    for (const p of toCheck) {
                                        ...
                                        // gridsOverlap даёт ложное срабатывание — сетка слишком
                                        // грубая, чтобы увидеть что дуга на внутреннем углу
                                        // вырезает материал в зоне перекрытия.
                                        // Используем positionedPolygons (точная геометрия с дугами).
                        ...
                        // v3.36: Ищем правый край в ряду последнего 0° элемента,
                        // чтобы продолжать текущий ряд после заполнения предыдущего.
                        // Раньше искали по ВСЕМ рядам → после заполнения первого ряда
                        // алгоритм не мог продолжить второй ряд и ставил детали
                        // в один столбец.

                        // ═══════════════════════════════════════════════════════════
                        // v4.30: ЦЕПОЧКА ТРЕУГОЛЬНИКОВ — 0°+180°+0°+180°+...
                        // Когда count0==count180 (пара завершена), для треугольников
                        // НЕ переходим на новый ряд сразу. Сначала пробуем вложить
                        // 0° в последний 180° (симметрично идеальной interlock-
                        // позиции). Это позволяет создать цепочку 0°-180°-0°-180°
                        // в одном ряду — 3+ треугольника вместо 2.
                        // ═══════════════════════════════════════════════════════════
                        if (isTriangle) {
                            ...
                            if (last180ForRow) {
                                ...
                                // 180°'s right edge = reflected 0°'s left edge.
                                // 0°'s left edge начинается в (leftApexX, 0) и идёт вверх-влево.
                                // После 180° rotation это ребро становится 180°'s right edge,
                                ...
                                // сканируем позиции СРАЗУ ПОСЛЕ 180° (вкладываясь в его
                                // правую сторону). Сканируем только вправо от 180°.
                                // v4.31: шаг 3мм (был 5мм) — находит оптимальную позицию
                                // точнее. Начинаем с 0.45*refW (было 0.55) — ближе к apex.
                        ...
                        if (last0ForRow) {
                            ...
                            // pairHeight ≈ rowH, gap между рядами = minGap,
                    ...
                    // Пробуем классические стратегии 1 и 2 (тот же угол)
                    if (nestCandidates.length === 0) {
                ...
                // ─── Выбираем лучший вариант (минимальный BLF-оценку) ───
                if (nestCandidates.length > 0) {
            ...
            // с первым элементом нового ряда [edgeGap, edgeGap+bboxWidth]
            ...
            for (const p of placedParts) {
                ...
                if (edgeGap < (p.x || 0) + pw + minGap && newPartXEnd + minGap > (p.x || 0)) {
                    ...
                    // bbox-перекрытие при расчёте нового ряда —
                    // детали могут вкладываться вогнутыми сторонами
            ...
            // паттерн <<<< реализуется через «вслед:вложение» выше.
            // Quick-tries используются как fallback (обычный «вслед», «новый ряд»).

            // «Под всеми» — только если отличается от «новый ряд»
            // (для смешанных типов: разные детали могут торчать ниже)
            ...
            for (const qt of quickTries) {
                ...
                // ═══════════════════════════════════════════════════
                // FAST-PATH для однотипных партий:
                // Если на листе только один тип деталей и «вслед»,
                // bbox-проверки достаточно — детальная не нужна.
                // «Новый ряд» всегда проверяем детально (может
                // перекрываться с деталями из предыдущих рядов).
                // v3.6: «вложение» — всегда детальная grid-проверка
                // ═══════════════════════════════════════════════════
                ...
                // simple-follow — hull не отражает реальные отверстия,
                // и bbox-проверки недостаточно (другие детали могут
                // попасть в пустоты). Также это предотвращает ложное
                // «в один рез» поведение для деталей с отверстиями.
                const hasHolesFlag = newPart._hasHoles === true;
                // v4.05: Для деталей с дугами (arc) тоже НЕ используем
                // simple-follow — convex hull не отражает кривизну дуг,
                ...
                for (const p of placedParts) {
                    ...
                    if (qt.x < p.x + pw + minGap && qt.x + qtPrepared.bbox.width + minGap > p.x &&
                        ...
                        // bbox-проверку (точная grid-проверка ниже)
                    ...
                    for (const p of toCheck) {
                        ...
                        if (needsGrid && p.positionedHull?.length) {
                            if (N.polygonsIntersect(phull, p.positionedHull, minGap)) {
                                ...
                                // (может быть ложное срабатывание из-за отверстий)
                                ...
                                if (newNeedsGrid || placedNeedsGrid) {
                                    ...
                                    // Аналогичный баг как в checkCandidates — когда только одна
                                    // деталь нуждается в grid, обе переменные ссылались на одну деталь.
                                    const pAngleDeg = Math.round((p.angle || 0) * 180 / Math.PI) % 360;
                                    if (N.gridsOverlap(newPart, qt.x, qt.y, qtPrepared.bbox.width, qtPrepared.bbox.height, qtAngleDeg,
    ...
    // ═══════════════════════════════════════════════════
    // NEIGHBOR QUICK-PLACE: для первого экземпляра нового
    // типа детали — пробуем разместить рядом с ближайшей
    // уже размещённой деталью (не только своего типа).
    // Это быстрее чем полный generateCandidates + Best-fit.
    // ═══════════════════════════════════════════════════
    if (placedParts.length > 0 && !isCircular) {
        ...
        if (!hasSameType) {
            ...
            if (nearest.part) {
                ...
                // углами (allowedAngles=[90°]) — нужно размещать при 90°, не 0°!
    ...
    // ═══════════════════════════════════════════════════
    // PRE-FILTER УГЛОВ: вычисляем copiesPerRow для каждого
    // угла и пропускаем те, что явно проигрывают.
    // Также кешируем prepareRotatedHull чтобы не вызывать
    // дважды для одного угла.
    // ═══════════════════════════════════════════════════
    const angleInfo = [];
    ...
    // ═══════════════════════════════════════════════════
    // v3.82: CIRCLE-IN-RING Quick-Place
    // Размещение маленьких круглых деталей ВНУТРИ
    // отверстий уже размещённых кольцевых деталей.
    // Приоритетнее обычного Circle-QP — заполняем
    // пустоты внутри колец, а не свободное пространство
    // листа. Пример: кольцо Ø420 (внутр. Ø326) →
    // внутри помещается 1-2 круга Ø150.
    // ═══════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════
    // v3.80: CIRCLE QUICK-PLACE
    // Для круглых деталей: размещаем следующую деталь
    // рядом с последней такого же типа на гексагональном
    // смещении. Это даёт плотную гекс-упаковку вместо
    // рассеянной раскладки через полный generateCandidates.
    // Раньше круглые детали пропускали Quick-Place
    // (!isCircular) и шли через Best-fit, который
    // генерировал позиции по всей сетке листа → детали
    // разбрасывались (gap=953мм и т.п.).
    // ═══════════════════════════════════════════════════
    if (isCircular && placedParts.length > 0) {
        const prepared = N.prepareRotatedHull(partHull, 0, centerX, centerY);
    ...
    // ═══════════════════════════════════════════════════
    // BEST-FIT: оцениваем отфильтрованные углы и выбираем
    // лучший. Критерий 1: BLF-score, Критерий 2: copiesPerRow
    // ═══════════════════════════════════════════════════
    let bestResult = null;
