// ════════════════════════════════════════════════════════════════
// SilikinK Nesting Engine — Rotation & Placement Helpers (Module 12)
// ════════════════════════════════════════════════════════════════
(function(N) {
    'use strict';
...
N.getRotationAngles = function getRotationAngles(part, isFirstPart) {
    ...
    // + автоматически добавляем комплементарные углы для лучшей плотности:
    //   180° → +0° (Г-образные детали: 0° и 180° взаимодополняют)
    //   135° → +45° (симметричная пара)
    // [getRotationAngles] part=${part.name}, allowedAngles=${JSON.stringify(part.allowedAngles)}, noRotate=${noRotate}
    if (part.allowedAngles && part.allowedAngles.length > 0) {
        ...
        // 180° комплементарен 0° (для Г-образных и несимметричных деталей)
    ...
    if (mode === 'fast') {
        ...
        // (Circle, Rect и т.д.) не имеют свойства .type, поэтому o.type === 'circle'
        // и o.type === 'rect' всегда возвращали false для canvas-деталей.
        // Это приводило к тому, что canvas-прямоугольники классифицировались
        // как «смешанная» деталь с 5 углами вместо 1-2, и spacing не применялся.
        ...
        // v3.67: N.getShapeType() вместо o.type — Text из shapes.js может не иметь .type
