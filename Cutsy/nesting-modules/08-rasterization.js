// ════════════════════════════════════════════════════════════════
// SilikinK Nesting Engine — Grid Rasterization (Module 08)
// ════════════════════════════════════════════════════════════════
(function(N) {
    'use strict';
...
N.mergeCloseVertices = function mergeCloseVertices(objects, eps = N.MERGE_EPS) {
    ...
    for (const obj of objects) {
        ...
        // (Circle, Rect, etc.) могут не иметь свойства .type
        ...
        // точке другой — сливаем (формируем замкнутый контур)
        // FIX: Оптимизация — используем spatial hash вместо O(n²) перебора.
        // Для больших DXF (2000+ линий) O(n²) давал 4M+ итераций.
        if (objType === 'line') {
...
N.rasterizePartToGrid = function rasterizePartToGrid(part, gridSize = 10) {
    ...
    // закрывает микрозазоры в DXF-контурах (0.01–0.02мм)
    // перед тем как flood-fill начнёт искать пустоты.
    // Мутирует objects in-place — это безопасно, т.к. объекты
    // загружаются один раз из DXF и не меняются после.
    ...
    for (let oi = 0; oi < (part.objects || []).length; oi++) {
        ...
        // могут не иметь свойства .type
        const objType = N.getShapeType(obj);
        if (objType === 'rect') {
            // v3.67: Убрана дублирующая растеризация — раньше rect заполнялся
            // дважды: inline (ниже) и через N.rasterizeRectToGrid(). Оставляем
            // только N.rasterizeRectToGrid(), как для circle/polygon.
...
N.rasterizeLineObjectToGrid = function rasterizeLineObjectToGrid(obj, bbox, grid, gw, gh, gridSize, options = {}) {
    const {
        ...
        expandBy = 1           // Расширение на N ячеек (для rasterizePartToGrid = 1)
    } = options;
    ...
    if (objType === 'line') {
        ...
        if (pts.length >= 3 && fillPolygons) {
            ...
            // v3.67: Для замкнутых полилиний рисуем замыкающий сегмент
            // (последняя→первая точка), иначе на контуре образуется надрез.
...
N.getFilledOccupancyGrid = function getFilledOccupancyGrid(part, gridSize = 10) {
    ...
    // v3.54: N.getShapeType() вместо obj.type — классы из shapes.js
    // могут не иметь свойства .type (Circle, Rect, Arc, Polygon)
    ...
    for (const obj of part.objects || []) {
        ...
        if (objType === 'line' || objType === 'arc' || objType === 'polyline' || objType === 'lwpolyline') {
            ...
            N.rasterizeLineObjectToGrid(obj, bbox, grid, gw, gh, gridSize, {
                fillPolygons: true,
                expandBy: 0  // Без расширения — только сама ячейка
            });
    ...
    // v3.36: Dilation закрывает 1-пиксельные щели на стыках линий/дуг,
    // предотвращая утечку flood-fill через разрывы контура.
    // Без dilation детали с вырезами на краях получают fill≈22%
    // вместо правильных ≈80%, что приводит к ложным пересечениям
    // (плотность >100%) и ошибочной классификации Г-образности.
    ...
    // Используем BFS для заполнения всех ячеек, достижимых
    // от границы сетки без пересечения линии контура.
    ...
    // ═══════════════════════════════════════════════════
    // v3.53: Порядок шагов 2.5-2.7 изменён!
    // Раньше: 2.5 → 2.6 → 2.7 (hull patch после hole detection)
    // Проблема: hull patch восстанавливает ячейки, которые
    // шаг 2.5/2.6 очистил как дыры (convex hull покрывает
    // отверстия). Для фланца (кольцо): шаг 2.5 очищает
    // внутренний круг → шаг 2.7 hull patch заполняет его
    // обратно → detectPartHoles не находит дыру.
    // Теперь: 2.7 (hull patch) → 2.5 (concentric) → 2.6 (holes)
    // Hull patch работает ДО очистки → не затирает дыры.
    // ═══════════════════════════════════════════════════

    // Шаг 2.7: v3.52 Hull-assisted interior patch.
    // Если flood-fill «протёк» через щели контура (микрозазоры
    // на стыках линий/дуг при конвертации LWPOLYLINE→lines+arcs),
    // внутренние ячейки ошибочно помечаются как exterior (2).
    // Определяем это по аномально низкому fillRate относительно
    // ожидаемого (hull area / bbox area) и восстанавливаем
    // внутренние ячейки через isPointInPolygon по hull полигону.
    {
    ...
    // Для кольца (два круга с общим центром) внешний круг заполняет
    // весь круг (1), включая область внутреннего круга. Flood fill
    // не может «пробиться» через внешний круг → внутренняя область
    // помечена как interior (0), а не как exterior (2).
    // На шаге 3 она стала бы 1 (материал) → hole-fill заблокирован.
    // Решение: определяем концентрические круги и очищаем
    // область внутреннего круга → 2 (exterior), чтобы на шаге 3
    // она стала 0 (пусто = дыра, куда можно вложить деталь).
    ...
    // вырезает как пустоту. Для концентрических кругов шаг 2.5
    // работает, но для круг-внутри-прямоугольника — нет.
    // Решение: определяем круги, полностью внутри rect, и очищаем
    // их область → 2 (exterior), чтобы на шаге 3 стало 0 (дыра).
    {
    ...
    // Для кольцеобразных деталей (ring/flange) внутренний контур
    // может состоять из ЛИНИЙ (а не circle-объектов). В этом случае
    // шаг 2.5 не очищает внутреннюю область, и flood-fill не может
    // «пробиться» через внутреннюю границу → область внутри кольца
    // остаётся как 0 (interior) → на шаге 3 становится 1 (материал)
    // → detectPartHoles не находит дыру → hole-fill не работает.
    //
    // Алгоритм:
    // 1. Находим «material seeds» — 0-ячейки, соседствующие с
    //    1-ячейками (граница), которые сами соседствуют с 2-ячейками
    //    (exterior). Это ячейки внутри ВНЕШНЕЙ границы = материал.
    // 2. Flood-fill через 0-ячейки от material seeds → подтверждённый материал.
    // 3. Оставшиеся 0-ячейки (не достигнутые flood-fill) = дыры.
    //    Помечаем их как 4 (hole) — на шаге 3 станут 0 (пусто).
    {
        ...
        for (let gy = 1; gy < gh - 1; gy++) {
            for (let gx = 1; gx < gw - 1; gx++) {
                ...
                // Проверяем: есть ли среди соседей 1-ячейка,
    ...
    // exterior (2) → 0, hole (4) → 0, boundary (1) + interior (0) → 1
    let materialCells = 0;
    let emptyCells = 0;
    for (let i = 0; i < grid.length; i++) {
    ...
    // В этом случае используем тонкий контур без заливки.
    // FIX #3: Добавляем guard lineCount > 5 — очень тонкие детали
    // (пружины, контуры букв, тонкие кронштейны) имеют мало
    // материала даже при правильном контуре. Без guard они
    // ошибочно попадают в fallback.
