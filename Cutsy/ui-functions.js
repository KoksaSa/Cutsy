...
function showProperties(obj) {
    console.log('🔍 [UI-FILES.JS] showProperties вызван', obj ? `type=${obj.type}` : 'obj=null');
...
function updateStatusBar() {
    console.log('📊 [updateStatusBar] Вызов из ui-functions.js');
...
window.updatePartsList = function updatePartsList() {
    console.log('📦 [updatePartsList] Вызов из ui-functions.js, деталей:', parts.length);
    ...
    list.querySelectorAll('.part-spacing-input').forEach(input => {
        input.addEventListener('change', (e) => {
            ...
            console.log(`📏 [ui-functions.js] Изменение зазора: Деталь #${partId} "${part.name}" → ${spacing} мм`);
            
            if (isNaN(spacing) || spacing < -10 || spacing > 500) {
                // Возвращаем значение по умолчанию 3 мм
                const defaultValue = (typeof part.spacing === 'number') ? part.spacing : 3;
                e.target.value = defaultValue;
                console.warn(`⚠️ Зазор ${spacing} вне диапазона [-10;500], установлено ${defaultValue} мм`);
                return;
            }
            
            part.spacing = spacing;
            if (typeof saveToCache === 'function') saveToCache();
            console.log(`✅ Зазор сохранён: ${spacing} мм`);
        });
