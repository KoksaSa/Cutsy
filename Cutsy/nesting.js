...
function findPositionWithCommonEdge(placedParts, newPart, sheetWidth, sheetHeight, minGap, edgeGap) {
    console.log(`📐 [FIND POSITION WITH COMMON EDGE] Поиск позиции с общей гранью для "${newPart.name || newPart.id}"`);
...
function generateCandidatePositions(placedParts, partWidth, partHeight, sheetWidth, sheetHeight, minGap, edgeGap, step) {
    ...
    // 1. Позиция в левом нижнем углу листа (базовая) - ТОЛЬКО если placedParts пуст
    ...
    for (let i = 0; i < rotationAngles.length; i++) {
        ...
        if (!isCircular) {
            ...
            for (let ci = 0; ci < allCandidates.length; ci++) {
                ...
                // ═══════════════════════════════════════════════════════
                // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: всегда используем placedParts
                // spatialGrid может быть пустым или некорректным
                // ═══════════════════════════════════════════════════════
                const partsToCheck = placedParts;  // Всегда проверяем ВСЕ размещённые детали
                ...
                for (const placed of partsToCheck) {
                    ...
                    // Сначала быстрая проверка по bounding box с зазором
                    const placedWidth = placed.width || placed.bboxWidth || 0;
                    const placedHeight = placed.height || placed.bboxHeight || 0;
                    ...
                    if (x + rotatedBbox.width + minGap <= placed.x ||
                        ...
                        if (placed.positionedHull && Array.isArray(placed.positionedHull)) {
                            ...
                            // Если нет positionedHull, проверяем по bbox без зазора
                            if (x < placed.x + placedWidth &&
            ...
            if (checkedCount > 0) {
                console.log(`      ❌ Поворот ${rotationAngles[i]}°: проверено ${checkedCount} позиций, отклонено: ${rejectedBySheet} (лист) + ${rejectedByIntersection} (пересечение)`);
            }
            
            // ═══════════════════════════════════════════════════════
            // КРИТИЧЕСКИ ВАЖНО: если ни одна позиция не подошла - возвращаем null!
            // ═══════════════════════════════════════════════════════
            if (checkedCount > 0 && rejectedByIntersection + rejectedBySheet === checkedCount) {
                console.log(`   ⛔ ВСЕ позиции отклонены для поворота ${rotationAngles[i]}° - переходим к следующему`);
                continue;  // Пробуем следующий поворот
            }
...
function findOptimalSheet(partsToPlace) {
    console.log(`📐 [FIND OPTIMAL SHEET] Поиск оптимального листа для ${partsToPlace.length} типов деталей`);
...
async function tryPlacePartsOnSheet(partsToPlace, sheetWidth, sheetHeight, minGap, cancelCallback = null) {
    ...
    console.log(`📦 [TRY PLACE] Начало размещения ${partsToPlace.length} типов деталей (${totalNeeded} шт) на листе ${sheetWidth}×${sheetHeight}, зазор: ${minGap}мм`);
...
function rotateNestedPart(nested, rotationDegrees, sheetWidth, sheetHeight, allNestedParts) {
    console.log(`🔄 [ROTATE NESTED PART] Поворот детали #${nested.partId} на ${rotationDegrees}°`);
