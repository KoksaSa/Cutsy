...
function exportSheetToDXF() {
    ...
    // Parts
    // Собираем определения деталей из текущего листа
    ...
    console.log(`   📋 Найдено определений деталей: ${Object.keys(singleSheetPartDefs).length}`);
    ...
    Object.keys(partsByType).forEach(partId => {
        ...
        partsByType[partId].forEach(nested => {
            console.log(`   📦 Экспорт детали #${partId} на (${nested.x},${nested.y}), угол=${(nested.angle||0).toFixed(1)}`);
    ...
    const fileName = `sheet_t${thickness}mm_${dt}.dxf`;
